...
def generate_launch_description():
    ...
    filepath_config_twist_mux = PathJoinSubstitution(
        [FindPackageShare('amiga_control'), 'config', 'twist_mux.yaml']
    )

    filepath_config_interactive_markers = PathJoinSubstitution(
        [FindPackageShare('amiga_control'), 'config', 'teleop_interactive_markers.yaml']
    )
    ...
    node_twist_mux = Node(
        ...
        output='screen',
        remappings={('/cmd_vel_out', '/amiga_velocity_controller/cmd_vel_unstamped')},
